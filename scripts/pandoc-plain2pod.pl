...
my %handler_for = (
    ...
    Link => sub {	# {{{2}}}
        ...
        $Nested += 1;
        # construct the opening delimiter
        my $odel = '<' x $Nested;
    ...
    Header => sub {	# {{{2}}}
        ...
        $contents->[-1] = [ _mk_elem( Para => $text ) ];  # Div contents must belist of Blocks
        return $elem;
    },

    Code => sub {	# {{{2}}}
        my( $elem ) = @_;
        return if 'raw_pod' eq $elem->{c}[-2][1][0]; # leave raw pod as is
        # local $Nested unless $Nested;
        # cf. Link handler_for code!
        my $orig_nested = $Nested;
        $Nested ||= 1;
        # $Nested += ( $elem->{c}[-1] =~ tr/<>/<>/ ) || 1;
        # get the width of the longest chunk of angle brackets in the code
        my $most_angular = $option->{escape_lt_gt} ? 1
        : max map { length $_ } $elem->{c}[-1] =~ /(\<+|\>+)/g;
        # bump the delimiter angle count to be higher
        $Nested += $most_angular || 1;
        my $odel = '<' x $Nested;
        (my $cdel = $odel) =~ tr/</>/;
        # Escape inner formatting codes
        $elem->{c}[-1] =~ s/([<>])/$angle2ent{$1}/g if $option->{escape_lt_gt};
        undef $Nested unless $orig_nested;
        # wrap code and delimiters in a span 'coz we must return a single hash
        return _mk_elem_with_class( 
            Span => [ _mk_pod_inline(qq(C$odel ) ), $elem, _mk_pod_inline(qq( $cdel) ), ], 'code',
        );
    },
    ...
    DefinitionList => sub {	# {{{2}}}
        ...
        for my $item ( @$list ) {  # an array of arrays
            ...
            for my $definition ( @$definitions ) {  # array of arrays of bocks
...
filter if you will. It works by replacing or wrapping certain Pandoc AST
elements with Span, Div and other elements with raw POD markup injected
into their contents lists as inline Code elements which the plain writer
renders as unadorned verbatim text.
